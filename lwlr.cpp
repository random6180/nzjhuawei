...
//	int servernum = 0;
	...
	for (unsigned int i = 0; i < flavorlist.size(); i++) {									//对flavor对象的num赋值，即预测过程
		...
		exceptionDetect(floatalllist[i], 4);
...
//			cout << "start:" << i << endl;
			result = preOneDay(sumflavorlist, sumflavorlist.size() + daynum + 1, 3+3*num);
//			cout << "result:" << result << endl;
			resultdays.push_back((int)(result));
			sumflavorlist.erase(sumflavorlist.begin());
