...
treenode* buildTree(vector<vector<float> >& inputvector, float feature=0.6) {							//可能会超时
	...
	int k = (int)(feature * (inputvector[0].size() - 1));

	while (k > 0) {
		int rd = rand() % (inputvector[0].size() - 1);
		if (find(colset.begin(), colset.end(), rd) == colset.end()) {
			colset.push_back(rd);
			k--;
		}
	}
	...
	node->left = buildTree(finalleftvector, feature);
	node->right = buildTree(finalrightvector, feature);
...
float rfPreOutput(vector<treenode*> nodevector, vector<float>& input) {
	float sum = 0.0;
	for (unsigned int i = 0; i < nodevector.size(); i++) {
		//		cout << "gbdt决策树" << i << ":" << preOutput(nodevector[i], input) << endl;
		sum += preOutput(nodevector[i], input);
	}
	return sum/nodevector.size();
}

vector<float> rfContinuesOutput(vector<treenode*> nodevector, vector<float>& input, int days) {		//预测几天输出
	vector<float> predays;
	float result;
	for (int i = 0; i < days; i++) {
		result = rfPreOutput(nodevector, input);
		predays.push_back(result);
		for (unsigned int j = 0; j < input.size() - 1; j++)			//对输入左移
			input[j] = input[j + 1];
		input[input.size() - 1] = result;
...
//		dimension = 7;
		...
		vector<treenode*> nodevector;
		nodevector = randomForest(inputvector, 50, 0.2);

		inputtest = getInput(floatalllist[i], dimension);


		resultdays = rfContinuesOutput(nodevector, inputtest, num);
